...
class ObjSeccioFrontendProducte {
    ...
    async iniciaSeccio (idProducte) {
        ...
        if (objRebut.resultat === 'ok' && objRebut.missatge.length === 1) {
            ...
            codiHTML = codiHTML + '<img src="' + valor.imatge + '" width="500" height="500px" />'
            codiHTML = codiHTML + '<h3 class="detall-nom">' + valor.nom +'</h3>'
            codiHTML = codiHTML + '<div class="detall-descripcio">' + valor.descripcio +'</div>'
            codiHTML = codiHTML + '<div class="detall-preu">' + valor.preu +' €</div>'
